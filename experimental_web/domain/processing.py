import pandas as pd
from experimental_web.domain.kinetic_model import KineticModel, InitConditions, OptimumTShift
import numpy as np
import traceback
...
class TableProcessor:
    ...
    def create_model(self,
        ...
        if max_time is not None:
            try:
                req_tmax = float(kwargs.get('t_max') or 0)
            except Exception:
                req_tmax = 0.0
            if req_tmax <= 0:
    ...
    def compute_all_kinetics(self,
        ...
        log.info(
            'compute_all_kinetics: initialization=%s t_shift=%s t_max=%s t_max_plot=%s optim_time_shift=%s models=%s custom_models=%s',
            getattr(initialization, 'name', str(initialization)),
            float(t_shift) if t_shift is not None else None,
            float(t_max) if t_max is not None else None,
            float(t_max_plot) if t_max_plot is not None else None,
            bool(optim_time_shift),
            len(list(models_to_compute or [])),
            len(list((custom_models or {}).keys())) if custom_models else 0,
        )
        def _emit(event: dict) -> None:
            """Optional progress hook; must never break the computation."""
        ...
        if c_model in models_to_compute:
            self.k_models[c_model] = self.create_model(self.tables['C18_1'],
                                                              ...
                                                              init_method=initialization, t_shift=t_shift, t_max=t_max)
        ...
        if c_model in models_to_compute:
            self.k_models[c_model] = self.create_model(self.tables['C18_1'],
                                                   ...
                                                   init_method=initialization, t_shift=t_shift, k_uh=False, t_max=t_max)
        ...
        if c_model in models_to_compute:
            self.k_models[c_model] = self.create_model(self.tables['C18_2'],
                                                             ...
                                                             init_method=initialization, t_shift=t_shift, k_uh=True, t_max=t_max)
        ...
        # merge in configurable / custom models (e.g. generated from computation graph)
        if custom_models:
            try:
                for _k, _m in dict(custom_models).items():
                    if _m is not None:
                        self.k_models[_k] = _m
            except Exception:
                pass
        ...
        if optim_time_shift:
            _emit({"type": "phase", "phase": "optim_time_shift", "state": "start"})
            with log_scope('OptimumTShift.fit', logger=log, level=logging.INFO):
                OTS = OptimumTShift(models=self.k_models, t_shift_init=t_shift, verbose=False)
                t_shift, t_shift_errorsum = OTS.fit()
            log.info('Optimální t_shift=%s; error_sum=%s', t_shift, t_shift_errorsum)
            _emit({
                "type": "phase",
                "phase": "optim_time_shift",
                "state": "end",
                "t_shift": float(t_shift),
                "error_sum": float(t_shift_errorsum) if t_shift_errorsum is not None else None,
            })

        total = len(self.k_models)
        for idx, (k, v) in enumerate(self.k_models.items(), start=1):
            _emit({"type": "model_start", "name": k, "idx": idx, "total": total})
            with log_scope('fit_model', logger=log, level=logging.DEBUG, name=k, idx=idx, total=total):
                v.reinit_kinetic_model(t_shift=t_shift)
                # Avoid noisy output unless explicitly enabled.
                v.verbose = log.isEnabledFor(logging.DEBUG)
                c_k, _ = v.fit()
            log.info('%s: %s', k, c_k)

            if post_fit_callback is not None:
                try:
                    post_fit_callback(k, v, idx, total)
                except Exception:
                    # progress reporting is optional and must never break the computation
                    pass
