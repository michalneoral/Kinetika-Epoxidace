...
from experimental_web.domain.constants import K_MIN, K_MAX
from experimental_web.domain.str_enum import StrEnum
...
class OptimumTShift:
    """Estimate a global time-shift (t_shift) for one or more kinetic models.

    Performance note
    ---------------
    A naive implementation would perform a *full* kinetic-constant fit for every
    evaluation of the outer t_shift optimiser (nested optimisation), which can
    easily make the "automatic time shift" path look stuck.

    We therefore:
    - use a bounded 1D scalar optimiser (`minimize_scalar(method='bounded')`),
    - perform only a *quick* inner fit with limited iterations (warm-started),
    - and rely on the main pipeline to do the final full fit once t_shift is found.
    """
    ...
    def __init__(
        ...
        verbose=True,
        inner_maxiter: int = 25,
        max_evals: int = 15,
    ):
        self.models = models
        self.t_shift_init = t_shift_init
        b0, b1 = t_shift_bounds
        self.t_shift_bounds = (float(b0), float(b1))
    ...
    def loss(self, t_shift):
        # SciPy passes ndarray sometimes; we want a scalar.
        t_shift = float(np.atleast_1d(t_shift)[0])

        fun_total = 0.0
        for _name, m in self.models.items():
            m.reinit_kinetic_model(t_shift=t_shift)

            # Quick fit only: cap inner iterations; warm-start using last fit.
            old_verbose = m.verbose
            m.verbose = False
            try:
                k_fit, fun = m.fit(maxiter=self.inner_maxiter, tol=1e-6)
                if k_fit is not None:
                    m.k_init = np.array(k_fit, dtype=np.float64)
            finally:
                m.verbose = old_verbose
        ...
        self._eval += 1
        if fun_total < self._best[1]:
            self._best = (t_shift, fun_total)

        # Throttle debug logs (avoid slowing down with spam).
        if log.isEnabledFor(logging.DEBUG) and (self._eval <= 3 or self._eval % 5 == 0):
            best_t, best_f = self._best
            log.debug(
                "OTS eval %d/%d: t_shift=%.4g loss=%.6g (best=%.4g -> %.6g)",
                self._eval,
                self.max_evals,
                t_shift,
                fun_total,
                (best_t if best_t is not None else float('nan')),
                best_f,
            )
    ...
    def fit(self):
        if not self.models:
            return float(self.t_shift_init), 0.0
        ...
        result = minimize_scalar(
            ...
            method='bounded',
            options={'maxiter': self.max_evals, 'xatol': 1e-3},
        )
        self.result = result
        self.t_shift_fit = float(result.x)
        if self.verbose:
            log.debug(
                'Optimal t_shift: %.6g (fun=%.6g; evals=%d)',
                self.t_shift_fit,
                float(result.fun),
                self._eval,
            )
        return self.t_shift_fit, float(result.fun)
...
class KineticModel:
    ...
    def __init__(self,
        ...
        # Optional override for graph-defined / configurable ODE models.
        self.param_names_override = list(param_names_override) if param_names_override is not None else None

        if custom_odes is not None:
            self.odes = custom_odes
            if self.param_names_override is not None and len(self.param_names_override) > 0:
                k_init_default = [0.01] * len(self.param_names_override)
            elif k_init is not None:
                k_init_default = [0.01] * len(list(k_init))
            else:
                raise ValueError(
                    "custom_odes vyžaduje buď param_names_override, nebo explicitní k_init (pro určení počtu parametrů)."
                )
    ...
    def get_constants_with_names(self):
        if self.odes is None:
            log.warning('No odes set yet')
            return None

        # Configurable (graph-defined) models: take names from override.
        if self.param_names_override is not None:
            def _latex_for_param(p: str) -> str:
                if not p:
                    return r"k"
                if p.startswith("k_"):
                    rest = p[2:]
            ...
            k_names = []
            for p in self.param_names_override:
                k_names.append({"name": p, "latex": _latex_for_param(p)})
            if self.k_fit is not None and len(self.k_fit) == len(k_names):
        ...
        if c_name.startswith('odes_k_'):
            ...
            log.warning('Unknown format of coefficients: %s', self.odes.__name__)
            return None
        ...
        if self.k_fit is None:
            if self.verbose:
                log.debug('Model is not fitted yet')
        elif len(k_split) == len(self.k_fit):
            for idx, k_value in enumerate(self.k_fit):
                k_names[idx]['value'] = float(k_value)
        else:
            log.warning('Model does not have the right number of kinetic constants')
        return k_names
    ...
    def fit(self, maxiter: int | None = None, tol: float = 1e-8):
        """
        ...
        self.k_fit = result.x
        if self.verbose:
            log.debug('Optimal coefficients: %s', self.k_fit)
    ...
    def plot_debug(self, sim_t=None, sim_y=None, ui=False, legend_mode="both", show_title=False, t_max_plot=None):
        """
        ...
        try:
            if t_max_plot is not None:
                mx = float(getattr(sim_t, "max", lambda: 0)()) if sim_t is not None else 0.0
                try:
                    mx_data = float(self.data[:, 0].max())
                except Exception:
                    mx_data = 0.0
                mx = max(mx, mx_data)
